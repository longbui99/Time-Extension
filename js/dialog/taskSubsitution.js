...
export class TaskSubsitution extends BaseDialog{
    startDateRef = this.useRef('start-date')
    endDateRef = this.useRef('end-date')
    searchBarRef = this.useRef('dialog-task-search-bar')
    ...
    renderDialogContent(){
        this.innerTemplate = `
            <div class="dialog-task-content">
                <div>
                    <div class="dialog-task-search-bar" l-ref="dialog-task-search-bar">
                    </div>
                </div>
                <div class="log-range" l-ref="dialog-task-log-range">
                ...
                <div>
                    <textarea rows="1" type="text" class="tm-form-control" placeholder="What are you doing?" l-ref="comment-for-task" tabindex="10004"></textarea>
                </div>
            </div>
        `;
        this.innerFooter = `
    ...
    async mounted(){
        ...
        this.searchBar._searchTask(`favorite  ${this.env.searchData.query}`, true)
        ...
        this.buttonConfirmRef.el.addEventListener('click', e=>{
            ...
            self.response.task = self.env.taskData;

class Main extends Component {
        ...
        if (true){
            ...
            for (let element of this.logHistoryRef.el.querySelectorAll('.tm-form-control')){
                element.addEventListener('change', event=>{
                    ...
                    self.do_invisible_request('POST', `${self.subEnv.serverURL}/management/ticket/work-log/update`, values);
                })
            }
            for (let element of this.logHistoryRef.el.querySelectorAll('.log-ticket')){
    ...
    async renderContent(refreshRelated=true, prefetch=true){
        ...
        function fetchGeneralData(){
            if (self.ticketData.localComment){
                self.commentRef.el.innerText = self.ticketData.localComment;
            }
            self.ticketData.displayName = self._getDisplayName(self.ticketData);
            self.searchRef.el.value = self.ticketData.displayName;
            self.el.querySelector('.ticket-navigation').style.display = "inline-block";
            self.ticketNavigator()
        }
        if (this.ticketData && prefetch){
            fetchGeneralData()
        } else {
